...
Create New User
    create session              MySession                           ${base_rul}
    ${body}=                    Create Dictionary                   firstName=test                  lastName=test1                    password=123123123      contact=123123123       username=berkali        type=administrator
    ${headers}=                 Create Dictionary                   fingerprint=1652435559794       authToken=${authToken}
    ${response}=                Post On Session                     MySession                       /users                            data=${body}        headers=${headers}
    log to console              ${response.json()}
    ${newUserId}=               Collections.Get From Dictionary     ${response.json()}              newUserId
    Set Global Variable         ${newUserId}
    log to console              ${response.status_code}
    log to console              ${response.content}
...
Get Single User
    create session              MySession                           ${base_rul}
    ${headers}=                 Create Dictionary                   fingerprint=1652435559794        authToken=${authToken}
    ${response}=                Get On Session                      MySession                        /users/${newUserId}/profile      headers=${headers}
    log to console              ${response.status_code}
    log to console              ${response.content}
